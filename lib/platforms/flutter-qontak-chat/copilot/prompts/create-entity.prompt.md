...
tools:
  ...
  - editFiles
  - search
description: "Create a Chat domain entity — @freezed, no Entity suffix, no fromJson. Runs type-query via RAG before writing each field to prevent duplicate types."
---
...
- **RAG (primary):** Call `search_code("<concept> enum value object", project_slug: "chat")` via the mobile-qontak MCP server. If a matching enum, value object, or typed ID is returned — use it.
- **Grep fallback:** If RAG is unavailable or returns no results, search the domain directories for the concept term.
